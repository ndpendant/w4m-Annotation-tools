...
if ( ( defined $mass ) and ( $mass ne "" ) and ( $mass =~ /[,]+/ ) ) {
	...
	my $ochemspider = lib::massbank_api->new() ;
	$string = $ochemspider->massbank_exe($CONF->{EXE}, $delta, $mass, undef, $CONF->{JSON_TMP}) ;
	...
	$string = $omassbank->massbank_exe($CONF->{EXE}, $delta, $mass, undef, $CONF->{JSON_TMP}) ;
...
## manage csv file containing list of masses (every thing is manage in jar)
	...
	my $omassbank = lib::massbank_api->new() ;
	$string = $omassbank->massbank_exe($CONF->{EXE}, $delta, undef, $masses_file, $col_mass, $col_id, $line_header ) ;
}
else {
...
if ( ( defined $out_html ) and ( defined $json ) ) {
	...
	($tbody_object) = $oHtml->add_entries_to_tbody_object($tbody_object, $CONF->{HTML_ENTRIES_PER_PAGE}, $masses, $ids, $json, $CONF->{'URL_MASSBANK'}) ;
...
if ( ( defined $out_tab ) and ( defined $json ) ) {
	...
	my $ocsv = lib::massbank_api->new() ;
	if (defined $masses_file) {
		if ( ( defined $line_header ) and ( $line_header == 1 ) ) { $massbank_matrix = $ocsv->set_massbank_matrix_object('massbank', $masses, $ids, $json ) ; }
		elsif ( ( defined $line_header ) and ( $line_header == 0 ) ) { $massbank_matrix = $ocsv->set_massbank_matrix_object(undef, $masses, $ids, $json ) ; }
		$massbank_matrix = $ocsv->add_massbank_matrix_to_input_matrix($complete_rows, $massbank_matrix) ;
		$ocsv->write_csv_skel(\$out_tab, $massbank_matrix) ;
	}
	elsif (defined $mass) {
		$massbank_matrix = $ocsv->write_csv_one_mass($masses, $ids, $json, $out_tab) ;
	}
} ## END IF
else {
...
print Dumper $massbank_matrix if $verbose == 3 ;
...
# wsdl_massbank is a script to identify mass/masses on massbank database webservices
...
wsdl_massbank.pl -- script for identification of mass/masses on massbank database webservices.
...
This script manage queries on ws massbank database.
