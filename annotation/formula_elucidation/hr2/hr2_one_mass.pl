...
###########################################################################################################
### hr2_one_mass.pl
### Input : -masse "237.5983 536.3254" -tolerance 1.0 -mode positif -charge 0 -regleOr NO -atome S,P -output1 sortieGalaxy.txt -output2 NoResult.txt -output3 conf.txt
...
# $ hr2_one_mass.pl -masse "237.5983 536.3254" -tolerance 1.0 -mode positif -charge 0 -regleOr NO -atome S,P -output1 sortieGalaxy.txt -output2 NoResult.txt -output3 conf.txt -outputView view.html
...
## si on met l'option -help on lance la fonction d'aide
if (defined($help)){
	help();
	exit;
}
...
open (OUTPUT1, '>'.$parametre{'output1'}) or die "can't create file output ".$parametre{'output1'}."\n" ;
	print OUTPUT1 "Id\t MassQuery\t Formula\t mmu\t MassTheoretical\t Rank\n";
close(OUTPUT1) ;
## permet de d'ecrire une premiere ligne de description dans le fichier de sortie
open (OUTPUT2, '>'.$parametre{'output2'}) or die "can't create file output ".$parametre{'output2'}."\n" ;
	print OUTPUT2 "Id\t MassQuery\t Formula\t mmu\t MassTheoretical\t Rank\n";
close(OUTPUT2) ;
...
## on parcourt les masses
my $Id = 0;
foreach my $masse (split (" " , $massesList)){
	$Id++;
	## verrification de la justesse de la masse et de la tolerance
	HR2::verrifMasse_Tolerance($Id, $masse, $parametre{'tolerance'}, \%parametre);
	
	## preparation de la commande d'execution de HR2 avec les paramettres definit dans %parametre
...
sub help {
	...
	# Created : 18/07/2012
	USAGE :
	        hr2_one_mass.pl -help
	        hr2_one_mass.pl -masse \"237.5983 536.3254\" -tolerance 1.0 -mode positif -charge 0 -regleOr NO -atome S,P -output1 sortieGalaxy.txt -output2 NoResult.txt -output3 conf.txt
	!!! ATTENTION !!! ce logiciel necessite :
		...
		- la presence dans le meme repertoire que masse_simple.pl de :
				- HR2-all-res.exe (executable de HR2 pour le systeme d'exploitation utiliser)
